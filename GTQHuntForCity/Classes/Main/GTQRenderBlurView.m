...
static NSString *GTQBlurCellIdentifier = @"blurCell";

@interface GTQRenderBlurView ()<UICollectionViewDataSource, UICollectionViewDelegate>
#ifdef iOS8
/** 用来模糊背景的特效 */
...
@implementation GTQRenderBlurView
...
{
    GTQRenderBlurView *view = [[self alloc] init];
    [view setImage:image];
    return view;
}
...
- (void)setupView {
    ...
    self.cancelBtn.frame = CGRectMake((ScreenWidth - 30) / 2, ScreenHeight - 64 - 50, 30, 30);
    ...
    CGFloat marginLR = (ScreenWidth - 3 * itemWH) / 4;
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"GTQBlurCell" bundle:nil] forCellWithReuseIdentifier:GTQBlurCellIdentifier];
}
...
{
    //取出cell对应的模型
    GTQFoundCellModel *foundModel = self.datas[0][indexPath.row];
    
    //取出cell
    GTQBlurCell *cell = [GTQBlurCell blurCellWithCollectionView:collectionView forIndexPath:indexPath identifier:GTQBlurCellIdentifier];
...
{
    GTQBlurCell *cell = (GTQBlurCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void)layoutSubviews
{
    ...
    //计算高度
    self.collectionView.frame = CGRectMake(0, self.bounds.origin.y + 60, ScreenWidth, self.bounds.size.height - 60 - 50);
}
@end
