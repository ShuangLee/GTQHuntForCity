...
#import "GTQMessageViewController.h"
#import "GTQMessageCell.h"
#import "GTQHaveNoMessageView.h"
#import "GTQMessageDeleteButton.h"
#import "GTQMessageRefreshHeader.h"
#import "MJRefresh.h"
#import "GTQMessageModel.h"
#import "GTQMessagePushViewController.h"

/** 底部删除按钮的高度 */
static const CGFloat deleteBtnHeight = 50.0;

@interface GTQMessageViewController ()<UITableViewDataSource, UITableViewDelegate, GTQMessageCellDelegate>
/** 消息tabelView */
@property (nonatomic, strong) UITableView *tableView;
/** 右边的导航按钮 */
@property (nonatomic, strong) UIButton *rightBtn;
/** 底部的删除全部按钮 */
@property (nonatomic, strong) GTQMessageDeleteButton *deleteAllBtn;
// 数据源
@property (nonatomic, strong) NSMutableArray *datas;
@end

@implementation GTQMessageViewController
...
- (NSMutableArray *)datas {
    if (_datas == nil) {
        ...
        for (NSDictionary *dict in tmepArr) {
            GTQMessageModel *model = [GTQMessageModel messageWithDict:dict];
...
- (void)setupUI {
    //添加没有消息的view
    GTQHaveNoMessageView *noMsg = [GTQHaveNoMessageView noHaveMessageView];
    ...
    GTQMessageRefreshHeader *header = [GTQMessageRefreshHeader headerWithRefreshingTarget:self refreshingAction:@selector(loadNewData)];
    // 隐藏时间
    ...
    //初始化底部删除按钮
    self.deleteAllBtn = [[GTQMessageDeleteButton alloc] initWithFrame:CGRectMake(0, ScreenHeight - 64, ScreenWidth, deleteBtnHeight)];
...
- (void)rightClick:(UIButton *)sender {
    ...
    for (NSInteger i = 0; i < self.datas.count; i++) {
        //取到cell
        GTQMessageCell *cell = (GTQMessageCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForItem:i inSection:0]];
        if (sender.selected) {
            [cell startEdit];
        } else {
            [cell endEdit];
...
{
    //这里我个人的逻辑是将所有的消息归档到本地,每点击删除一条，将本地的数据删除一条
...
{
    GTQMessageModel *model = self.datas[indexPath.row];
...
{
    GTQMessageCell *cell = [GTQMessageCell cellWithTableView:tableView NSIndexPath:indexPath];
...
{
    GTQMessagePushViewController *pushVC = [[GTQMessagePushViewController alloc] init];
